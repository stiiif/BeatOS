...
export function getEngineColor(track) {
    ...
    if (track.customSample) {
        // Has a custom/imported sample (via SMP, SRC, or Freesound)
        return COLORS.sample;
    }

    if (track.buffer) {
        // Has a generated buffer (KIK, SNR, HAT, FM) but no customSample
        return COLORS.generated;
    }

    // No engine assigned yet
    return COLORS.none;
}
...
/**
 * Check if a track has an engine assigned (buffer loaded or 909/automation type).
 */
