...
export class GranularLogic {
     ...
     * @param {number} time - Current audio time (for LFOs and default scan)
     * @param {number|null} scanTime - Optional specific time for scan calculation. If null, uses 'time'.
     * @returns {Object} result - { absPos, actStart, actEnd }
     */
    static calculateEffectivePosition(params, mod, time, scanTime = null) {
        ...
        // Use specific scanTime if provided (e.g. 0 for ResetOnTrig), otherwise global time
        const effectiveScanTime = (scanTime !== null) ? scanTime : time;
        const scanOffset = (currentScanSpeed * effectiveScanTime);
        ...
        if (windowSize > 0.0001) {
            // Calculate relative position from start of window
            let relativePos = anchorPos - actStart;
            
            // Add scan offset
            let relativePosWithScan = relativePos + scanOffset;
            
            // Perform wrap strictly within [0, windowSize]
            // (a % n + n) % n handles negative numbers correctly
