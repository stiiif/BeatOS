...
export class Mixer {
    constructor(containerSelector, trackManager, audioEngine) {
        ...
        this.onSelectTrack = null; // New callback for focus request
    ...
    setCallbacks(onMute, onSolo, onMuteGroup, onSoloGroup, onSelectTrack) {
        this.onMute = onMute;
        this.onSolo = onSolo;
    ...
    render() {
        ...
        tracks.forEach(track => mixerContainer.appendChild(this.createTrackStrip(track)));
        for(let i=0; i<4; i++) mixerContainer.appendChild(this.createGroupStrip(i));
        ...
        if (this.audioEngine.getContext()?.state === 'running' && this.isMetering && this.isVisible) {
             this.animateMeters();
        }
    }
    ...
    animateMeters(timestamp) {
        ...
        this.meterRegistry.forEach((meta, id) => {
            ...
            if (rawValue >= meta.targetValue) meta.targetValue = rawValue;
            else { meta.targetValue -= this.decayRate; if (meta.targetValue < 0) meta.targetValue = 0; }
            ...
            for(let i=0; i<numLeds; i++) {
                if(meta.currentValue >= this.ledThresholds[i]) activeLeds = i + 1;
                else break;
            }
    ...
    clearMeters() {
        if(this.meterCtx && this.meterOverlay) this.meterCtx.clearRect(0, 0, this.meterOverlay.width, this.meterOverlay.height);
    }
    ...
    createKnob(label, value, min, max, step, onChange, colorClass = '', showLabel = false) {
        ...
        knobOuter.addEventListener('mousedown', (e) => {
            isDragging = true; startY = e.clientY; startVal = currentValue;
            wrapper.classList.add('dragging'); document.body.style.cursor = 'ns-resize'; 
            e.preventDefault();
        ...
        const onMouseMove = (e) => {
            ...
            if (newVal !== currentValue) { currentValue = newVal; updateRotation(currentValue); onChange(currentValue); }
        };

        const onMouseUp = () => {
            isDragging = false; wrapper.classList.remove('dragging');
            document.body.style.cursor = ''; document.removeEventListener('mousemove', onMouseMove);
        ...
        knobOuter.addEventListener('dblclick', () => {
            ...
            currentValue = target; updateRotation(currentValue); onChange(currentValue);
        });
    ...
    createFaderSection(busObject, onChange, idForMeter) {
        ...
        muteBtn.className = 'mixer-btn mute'; muteBtn.innerText = 'M';
        const soloBtn = document.createElement('button');
        soloBtn.className = 'mixer-btn solo'; soloBtn.innerText = 'S';
        ...
        if (busObject) {
            if (busObject.analyser) analyserNode = busObject.analyser;
            else if (busObject.bus?.analyser) analyserNode = busObject.bus.analyser;
        }

        if (analyserNode) {
            const bufferLength = analyserNode.frequencyBinCount;
            this.meterRegistry.set(idForMeter, {
                el: wrapper, analyser: analyserNode, dataArray: new Uint8Array(bufferLength),
        ...
        fader.type = 'range'; fader.className = 'v-fader';
        fader.min = 0; fader.max = 1.2; fader.step = 0.01;
        fader.value = busObject?.params?.volume ?? busObject?.volume?.gain.value ?? 1.0;
    ...
    createLabelStrip() {
        ...
        widthSlider.type = 'range'; widthSlider.className = 'width-slider';
        widthSlider.min = 18; widthSlider.max = 42; widthSlider.value = 42;
        widthSlider.oninput = (e) => this.setTrackStripWidth(e.target.value);
        header.appendChild(widthSlider); strip.appendChild(header);

        const controls = document.createElement('div');
        controls.className = 'strip-controls';
        const createLabel = (text) => {
            const div = document.createElement('div'); div.className = 'mixer-pot label-item'; 
            const lbl = document.createElement('label'); lbl.innerText = text;
            div.appendChild(lbl); return div;
        };
        controls.appendChild(createLabel('Gain')); controls.appendChild(createLabel('Hi'));
        controls.appendChild(createLabel('Mid')); controls.appendChild(createLabel('Freq'));
        controls.appendChild(createLabel('Low')); controls.appendChild(createLabel('Snd A'));
        controls.appendChild(createLabel('Snd B')); controls.appendChild(createLabel('Drive'));
        controls.appendChild(createLabel('Comp')); controls.appendChild(createLabel('Pan'));
    ...
    createTrackStrip(track) {
        ...
        const header = document.createElement('div');
        header.className = 'strip-header cursor-pointer hover:bg-neutral-700 transition-colors';
        header.innerHTML = `<span class="strip-num">${track.id + 1}</span>`;
        // FEATURE REQUEST 1: Focus Right Panel on click
        header.onclick = () => { if (this.onSelectTrack) this.onSelectTrack(track.id); };
        ...
        controls.appendChild(this.createKnob('Gain', track.params.gain || 1, 0, 2, 0.01, (v) => { track.params.gain = v; const bus = getBus(); if(bus?.trim) bus.trim.gain.value = v; }, 'knob-color-green'));
        controls.appendChild(this.createKnob('Hi', track.params.eqHigh || 0, -15, 15, 0.1, (v) => { track.params.eqHigh = v; const bus = getBus(); if(bus?.eq?.high) bus.eq.high.gain.value = v; }, 'knob-color-blue'));
        controls.appendChild(this.createKnob('Mid', track.params.eqMid || 0, -15, 15, 0.1, (v) => { track.params.eqMid = v; const bus = getBus(); if(bus?.eq?.mid) bus.eq.mid.gain.value = v; }, 'knob-color-green'));
        controls.appendChild(this.createKnob('Freq', track.params.eqMidFreq || 1000, 200, 5000, 10, (v) => { track.params.eqMidFreq = v; const bus = getBus(); if(bus?.eq?.mid) bus.eq.mid.frequency.value = v; }, 'knob-color-green'));
        controls.appendChild(this.createKnob('Lo', track.params.eqLow || 0, -15, 15, 0.1, (v) => { track.params.eqLow = v; const bus = getBus(); if(bus?.eq?.low) bus.eq.low.gain.value = v; }, 'knob-color-red'));
        controls.appendChild(this.createKnob('A', track.params.sendA || 0, 0, 1, 0.01, (v) => { track.params.sendA = v; if(track.bus?.sendA) track.bus.sendA.gain.value = v; }, 'knob-color-yellow'));
        controls.appendChild(this.createKnob('B', track.params.sendB || 0, 0, 1, 0.01, (v) => { track.params.sendB = v; if(track.bus?.sendB) track.bus.sendB.gain.value = v; }, 'knob-color-yellow'));
        controls.appendChild(this.createKnob('Drive', track.params.drive || 0, 0, 1, 0.01, (v) => { track.params.drive = v; const bus = getBus(); if(bus?.drive?.input) this.audioEngine.setDriveAmount(bus.drive.input, v); }, 'knob-color-red'));
        controls.appendChild(this.createKnob('Comp', track.params.comp || 0, 0, 1, 0.01, (v) => { track.params.comp = v; const bus = getBus(); if(bus?.comp) this.audioEngine.setCompAmount(bus.comp, v); }, 'knob-color-purple'));
        controls.appendChild(this.createKnob('Pan', track.params.pan, -1, 1, 0.01, (v) => { track.params.pan = v; if(getBus()?.pan) getBus().pan.pan.value = v; }, 'knob-color-blue'));

        strip.appendChild(controls);
        const faderComp = this.createFaderSection(track, (v) => {
            track.params.volume = v; const bus = getBus(); if(bus?.vol) bus.vol.gain.value = v;
        }, track.id); 
    ...
    createGroupStrip(index) {
        ...
        // Select first track of group when group header is clicked
        header.onclick = () => { if (this.onSelectTrack) this.onSelectTrack(index * TRACKS_PER_GROUP); };
        strip.appendChild(header);

        const controls = document.createElement('div');
        controls.className = 'strip-controls'; 
        controls.appendChild(this.createKnob('Comp', 0, 0, 1, 0.01, (v) => {
            const bus = getBus(); if(bus?.comp) this.audioEngine.setCompAmount(bus.comp, v);
        ...
        this.driveModes.forEach(mode => { const opt = document.createElement('option'); opt.value = mode; opt.innerText = mode; driveSel.appendChild(opt); });
        driveSel.onchange = (e) => {
            const bus = getBus(); if(bus?.drive?.shaper) bus.drive.shaper.curve = this.audioEngine.driveCurves[e.target.value];
        };
        driveDiv.appendChild(driveSel);
        driveDiv.appendChild(this.createKnob('Amt', 0, 0, 1, 0.01, (v) => {
            const bus = getBus(); if(bus?.drive?.input) this.audioEngine.setDriveAmount(bus.drive.input, v);
        }, 'knob-color-red', true));
        ...
        sendSec.appendChild(this.createKnob('A', 0, 0, 1, 0.01, (v) => { const bus = getBus(); if(bus?.sendA) bus.sendA.gain.value = v; }, 'knob-color-yellow', true));
        sendSec.appendChild(this.createKnob('B', 0, 0, 1, 0.01, (v) => { const bus = getBus(); if(bus?.sendB) bus.sendB.gain.value = v; }, 'knob-color-yellow', true));
        ...
        ['HI', 'MID', 'LO'].forEach(band => {
            const btn = document.createElement('button'); btn.className = 'mixer-btn kill'; btn.innerText = band;
            btn.onclick = () => {
                btn.classList.toggle('active'); const isKill = btn.classList.contains('active'); const gainVal = isKill ? -40 : 0; const bus = getBus();
                if(bus?.eq) {
        ...
        eqSec.appendChild(killRow); controls.appendChild(eqSec);
        strip.appendChild(controls);

        const faderComp = this.createFaderSection(this.audioEngine.groupBuses[index], (v) => {
            const bus = getBus(); if(bus?.volume) bus.volume.gain.value = v;
        }, `group_${index}`);
    ...
    createReturnStrip(busObject, index) {
        const strip = document.createElement('div');
        // FEATURE REQUEST 3: Remove glossy border, change bg slightly
        strip.className = 'mixer-strip return-strip'; 
        strip.style.backgroundColor = index === 0 ? '#1a221f' : '#221a22'; 
        strip.style.borderColor = 'transparent'; 
        ...
        controls.appendChild(this.createKnob('Gain', 1.0, 0, 2, 0.01, (v) => { if(getBus().input) getBus().input.gain.value = v; }, 'knob-color-green', true));
        controls.appendChild(this.createKnob('Hi', 0, -15, 15, 0.1, (v) => { if(getBus().eq) getBus().eq.high.gain.value = v; }, 'knob-color-blue', true));
        controls.appendChild(this.createKnob('Mid', 0, -15, 15, 0.1, (v) => { if(getBus().eq) getBus().eq.mid.gain.value = v; }, 'knob-color-green', true));
        controls.appendChild(this.createKnob('Freq', 1000, 200, 5000, 10, (v) => { if(getBus().eq) getBus().eq.mid.frequency.value = v; }, 'knob-color-green', true));
        controls.appendChild(this.createKnob('Lo', 0, -15, 15, 0.1, (v) => { if(getBus().eq) getBus().eq.low.gain.value = v; }, 'knob-color-red', true));
        controls.appendChild(this.createKnob('A', 0, 0, 1, 0.01, (v) => { if(getBus().sendA) getBus().sendA.gain.value = v; }, 'knob-color-yellow', true));
        controls.appendChild(this.createKnob('B', 0, 0, 1, 0.01, (v) => { if(getBus().sendB) getBus().sendB.gain.value = v; }, 'knob-color-yellow', true));
        controls.appendChild(this.createKnob('Drive', 0, 0, 1, 0.01, (v) => { if(getBus().drive) this.audioEngine.setDriveAmount(getBus().drive.input, v); }, 'knob-color-red', true));
        controls.appendChild(this.createKnob('Comp', 0, 0, 1, 0.01, (v) => { if(getBus().comp) this.audioEngine.setCompAmount(getBus().comp, v); }, 'knob-color-purple', true));
        controls.appendChild(this.createKnob('Pan', 0, -1, 1, 0.01, (v) => { if(getBus().pan) getBus().pan.pan.value = v; }, 'knob-color-blue', true));

        strip.appendChild(controls);
        const faderComp = this.createFaderSection(busObject, (v) => { if(busObject.volume) busObject.volume.gain.value = v; }, `return_${index}`);
        faderComp.muteBtn.onclick = () => { const isMuted = faderComp.muteBtn.classList.toggle('active'); busObject.volume.gain.value = isMuted ? 0 : faderComp.fader.value; };
    ...
    createMasterStrip() {
        ...
        limiterDiv.className = 'mixer-pot'; limiterDiv.style.height = "auto";
        limiterDiv.innerHTML = `<label style="color:#ef4444; font-size:0.5rem;">LIMITER</label><div class="knob-outer" style="opacity:0.8; width:28px; height:28px;"><div class="knob-inner" style="transform:rotate(135deg)"><div class="knob-line" style="background:#ef4444;box-shadow:0 0 5px red"></div></div></div>`;
        controls.appendChild(limiterDiv); strip.appendChild(controls);
        const faderComp = this.createFaderSection(this.audioEngine.masterBus, (v) => { if(this.audioEngine.masterBus?.volume) this.audioEngine.masterBus.volume.gain.value = v; }, 'master');
        faderComp.muteBtn.style.visibility = 'hidden'; faderComp.soloBtn.style.visibility = 'hidden';
        strip.appendChild(faderComp.section);
        return strip;
    }
}