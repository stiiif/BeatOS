...
export class UIManager {
    ...
    initUI(addTrackCallback, addGroupCallback, visualizerCallback = null) {
        ...
        this.grid.initializeGrid(addTrackCallback, addGroupCallback, () => this.getSelectedTrackIndex(), (trk, step) => this.toggleStep(trk, step));
        this.automationPanel.initialize((idx) => this.setSelectedLfoIndex(idx), () => this.updateLfoUI());
        this.grooveControls.setCallbacks((timeSig) => this.updateGridVisuals(timeSig), (idx, visCb) => this.selectTrack(idx, visCb || visualizerCallback), () => this.getSelectedTrackIndex());
        this.grooveControls.initGrooveControls();
        this.syncGridElements();

        if (this.trackManager?.audioEngine) {
            ...
            this.mixer.setCallbacks(
                ...
                (trackId) => this.selectTrack(trackId, this.visualizerCallback) // FEATURE REQUEST 1: Sync mixer selection
            );
            this.mixer.render();
        }

        this.tracks.forEach(t => this.appendTrackRow(t.id, visualizerCallback));
    ...
    appendTrackRow(trk, visualizerCallback = null) {
        const info = this.trackOps.getRandomChokeInfo();
        this.grid.appendTrackRow(trk, visualizerCallback, info.mode, info.groups, (t, s) => this.toggleStep(t, s), (t) => this.toggleMute(t), (t) => this.toggleSolo(t), (t) => this.toggleStepLock(t), (g) => this.toggleMuteGroup(g), (g) => this.toggleSoloGroup(g), (t) => this.clearTrack(t), (g) => this.clearGroup(g), (t) => this.toggleIgnoreRandom(t), (t) => this.toggleIgnoreVelocityParams(t), (t) => this.randomizeTrackPattern(t), (t, cb) => this.selectTrack(t, cb));
        this.syncGridElements();
        if(this.mixer) this.mixer.render();
    }
    ...
    toggleStep(trk, step) {
        const info = this.trackOps.getRandomChokeInfo();
        this.grid.toggleStep(trk, step, () => this.trackOps.applyRandomChokeDimming(), info.mode);
    }
    ...
    selectTrack(idx, visualizerCallback = null) {
        const info = this.trackOps.getRandomChokeInfo();
        this.trackControls.setRandomChokeInfo(info.mode, info.groups);
    ...
    applyGroove() { this.grooveControls.setVisualizerCallback(this.visualizerCallback); this.grooveControls.applyGroove((timeSig) => this.updateGridVisuals(timeSig), (idx, cb) => this.selectTrack(idx, cb)); }
    async applyGrooveFreesound() { this.grooveControls.setVisualizerCallback(this.visualizerCallback); await this.grooveControls.applyGrooveFreesound((timeSig) => this.updateGridVisuals(timeSig), (idx) => this.selectTrack(idx), this.getSelectedTrackIndex()); }
    toggleMute(trk) { this.trackOps.toggleMute(trk); if(this.mixer) this.mixer.updateTrackState(trk); }
    toggleMuteGroup(grpIdx) { this.trackOps.toggleMuteGroup(grpIdx); if(this.mixer) this.mixer.updateAllTrackStates(); }
    toggleSolo(trk) { this.trackOps.toggleSolo(trk); if(this.mixer) this.mixer.updateTrackState(trk); }
    toggleSoloGroup(grpIdx) { this.trackOps.toggleSoloGroup(grpIdx); if(this.mixer) this.mixer.updateAllTrackStates(); }
    ...
    toggleSnapshot() { this.snapshotManager.toggleSnapshot((trk) => { this.updateTrackStateUI(trk); if(this.mixer) this.mixer.updateTrackState(trk); }, (idx, cb) => this.selectTrack(idx, cb), this.getSelectedTrackIndex(), this.visualizerCallback); }
